...
namespace RedflyDatabaseSyncProxy
{
    internal class SqlServerDatabasePrep
    {
        ...
        internal static LiteSqlServerDatabaseDocument? WorkingDatabase { get; private set; }

        internal static bool ForChangeManagement()
        {
            if (!SelectDatabaseFromLocalStorage())
            {
                if (!GetDatabaseDetailsFromUser())
                {
                    return false;
                }
            }
        ...
        private static bool GetDatabaseDetailsFromUser()
        {
            string serverName = "";
            string databaseName = "";
            string userName = "";
            ...
            WorkingDatabase = SaveDatabaseDetailsToLocalStorage(serverName, databaseName, userName, password);

            return (WorkingDatabase != null);
        }

        private static bool SelectDatabaseFromLocalStorage()
        {
            using (var db = new LiteDatabase(RedflyLocalDatabase.FilePath))
            {
                ...
                else
                {
                    ...
                    WorkingDatabase = all.ElementAt(selectedIndex - 1);
                    return true;
                }
            }
        }
