...
// Strongly-typed model for [dbo].[ErrorLog] generated by the
// redfly SqlServerPolyLangCompiler on 05/24/25 12:12:55 AM
// This is only meant to be indicative of the features available in the core product.
public class ErrorLog : TableEntityBase
{
...
public class ErrorLogDataSource : TableDataSourceBase<ErrorLog>
{
    ...
    protected override ErrorLog MapRowToTableEntity(Row row)
    {
        ...
        return new ErrorLog
        {
            ...
            ErrorSeverity = dict.TryGetValue("ErrorSeverity", out var v5) && !string.IsNullOrEmpty(v5) ? (int)Convert.ChangeType(v5, typeof(int)) : null,

            ErrorState = dict.TryGetValue("ErrorState", out var v6) && !string.IsNullOrEmpty(v6) ? (int)Convert.ChangeType(v6, typeof(int)) : null,

            ErrorProcedure = dict.TryGetValue("ErrorProcedure", out var v7) ? v7 : null,

            ErrorLine = dict.TryGetValue("ErrorLine", out var v8) && !string.IsNullOrEmpty(v8) ? (int)Convert.ChangeType(v8, typeof(int)) : null,
    ...
    protected override Row MapTableEntityToRow(ErrorLog entity, DbOperationType dbOperationType)
    {
        ...
        row.Entries.Add(new RowEntry { Column = "ErrorSeverity", Value = new Value { StringValue = entity.ErrorSeverity != null ? entity.ErrorSeverity.ToString() : null } });
        row.Entries.Add(new RowEntry { Column = "ErrorState", Value = new Value { StringValue = entity.ErrorState != null ? entity.ErrorState.ToString() : null } });
        row.Entries.Add(new RowEntry { Column = "ErrorProcedure", Value = new Value { StringValue = entity.ErrorProcedure } });
        row.Entries.Add(new RowEntry { Column = "ErrorLine", Value = new Value { StringValue = entity.ErrorLine != null ? entity.ErrorLine.ToString() : null } });
